    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-5 rounded-2xl bg-gradient-to-r from-slate-900/90 via-slate-900/60 to-amber-950/30 border border-slate-800 shadow-xl">
        <div className="space-y-1">
          ...
          <h2 className="text-xl sm:text-2xl font-bold text-white">Live Dispatched Booking Queue</h2>
          <p className="text-xs sm:text-sm text-slate-400">
            Appointments, food deliveries, and emergency rescue units captured with cryptographic integrity.
          </p>
        </div>
      ...
      <Vani3DCard glowColor="amber" className="p-4 sm:p-5 space-y-4">
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Filter by caller name, ticket ID, or service type..."
        ...
        <div className="space-y-3 max-h-[520px] overflow-y-auto pr-1">
          {filtered.length === 0 ? (
            <div className="py-12 text-center text-slate-500 text-xs">
              No dispatch tickets match the current filter.
            </div>
          ) : (
            filtered.map((t) => (
              <div
                key={t.ticketId}
                className="p-4 bg-slate-950/80 rounded-xl border border-slate-800 space-y-2 hover:border-slate-700 transition-all text-xs"
              >
                ...
                <div className="text-slate-300 font-medium">{t.serviceType}</div>
