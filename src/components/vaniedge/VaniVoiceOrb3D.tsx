...
import { VANI_THEMES, VaniTheme, hexToRgba } from "@/lib/vaniedge/theme-config";
  ...
  useEffect(() => {
    ...
    const render = () => {
      try {
        ...
        // Draw subtle background radial glow with safe RGBA parsing
        ...
        glowGrad.addColorStop(0, hexToRgba(glowColor, 0.22));
        ...
        for (let i = 0; i < projected.length; i += 4) {
          ...
          for (let j = i + 1; j < Math.min(i + 4, projected.length); j++) {
            ...
            if (dist < 44) {
              const alpha = Math.max(0.05, Math.min(0.4, (1 - dist / 44) * 0.38));
              ctx.strokeStyle =
                isSpeaking || isListening
                  ? hexToRgba(activeThemeConfig.primaryHex, alpha)
        ...
        for (const p of projected) {
          const alpha = Math.max(0.25, Math.min(1.0, (p.scale - 0.4) * 1.35));
        ...
        // Draw counter-rotating equatorial 3D gyroscope rings with valid RGBA
        ctx.lineWidth = 1.6;
        const ringRadius = radius * 1.1 + effectiveAmp * 0.6;
        ctx.strokeStyle = isSpeaking || isListening
          ? hexToRgba(activeThemeConfig.primaryHex, 0.45)
          : hexToRgba(activeThemeConfig.secondaryHex, 0.25);
        ...
        console.error("3D Orb render error:", err);
      }
